...
func _update_multimeshes() -> void:
	...
	for item in items:
		...
		for i in count:
			...
			var shape = BoxShape3D.new()  # Usa la forma adecuada para tu modelo
			shape.extents = mmi.get_aabb().size / 2
			collision_shape.shape = shape
			static_body.add_child(collision_shape)
			collision_shape.translation = Vector3.ZERO  # Ajusta según sea necesario
...
func _create_instance(item: ProtonScatterItem, root: Node3D):
	...
	var collision_shape = CollisionShape3D.new()
	var shape = BoxShape3D.new()  # Usa la forma adecuada para tu modelo
	shape.extents = instance.get_aabb().size / 2
	collision_shape.shape = shape
